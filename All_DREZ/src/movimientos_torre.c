...
bool es_abajo_de_torre(int fila_origen, int columna_origen, int fila_destino, int columna_destino, char campo[LADO][LADO]) {
	return (campo[fila_origen][columna_origen] == 't') ? fila_origen < fila_destino && columna_origen == columna_destino :
		fila_origen > fila_destino && columna_origen == columna_destino;
}
...
bool es_a_la_izquierda_de_torre(int fila_origen, int columna_origen, int fila_destino, int columna_destino, char campo[LADO][LADO]) {
	return (campo[fila_origen][columna_origen] == 't') ? columna_origen > columna_destino && fila_origen == fila_destino :
		columna_origen < columna_destino && fila_origen == fila_destino;
}
...
bool puede_ir_arriba_torre(int fila_origen, int fila_destino, int columna, char campo[LADO][LADO]) {
	int i = 0, distancia = 0;
	if(campo[fila_origen][columna] == 't') {
		distancia = fila_origen - fila_destino;
		for(i = 1 ; i < distancia ; i++) {
			if(campo[fila_origen - i][columna] != ' ') break;
		}
	} else {
...
bool puede_ir_izquierda_torre(int columna_origen, int columna_destino, int fila, char campo[LADO][LADO]) {
	int i = 0, distancia = 0;
	if(campo[fila][columna_origen] == 't') {
		distancia = columna_origen - columna_destino;
		for(i = 1 ; i < distancia ; i++) {
			if(campo[fila][columna_origen - i] != ' ') break;
		}
	} else {
...
bool es_jaque_torre(char pieza, int fila_destino, int columna_destino, int f_rey_b, int c_rey_b, int f_rey_n, int c_rey_n, char campo[LADO][LADO]) {
	bool es_jaque = false;
		if(pieza == 't' && movimiento_permitido_torre(fila_destino, columna_destino, f_rey_n, c_rey_n, campo)) es_jaque = true;
		if(pieza == 'T' && movimiento_permitido_torre(fila_destino, columna_destino, f_rey_b, c_rey_b, campo)) es_jaque = true;
	return es_jaque;

//	return (pieza == 't' && movimiento_permitido_torre(fila_destino, columna_destino, f_rey_n, c_rey_n, campo)) ||
//		(pieza == 'T' && movimiento_permitido_torre(fila_destino, columna_destino, f_rey_b, c_rey_b, campo));
